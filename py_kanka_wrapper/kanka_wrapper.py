...
"""
   NOTES:
   - This class should have very little to do with formatting the results
   of the queries.  That should be left to the calling logic since display
   format is usually dependant on context.

   - Not all functionality of the api is addressed or leveraged in this
   class.  I want to establish a working minimal product before filling in
   the rest and as it is currently meets my needs.  Code and trying to
   utilize rolls via an api is awkward.
"""
...
class kanka_wrapper:
    ...
    def __init__(self, token, campaign):
        ...
        self.current_entity = {}
        requests_cache.install_cache(campaign + '_cache',
                                     backend='memory',
                                     expire_after=300)
    ...
    def check_entity(self, entity_type, entity):
        """
            Helper method to support referring to entities
            by name or by id
        """
        if isinstance(entity, str):
            """
                This is an entity name so we have to look up the id.
                This should now translucently cache any requests via
                requests_cache.
            """
    ...
    def check_attribute(self, entity_type, entity, attribute):
        ...
        if isinstance(attribute, str):
            """
                This is an attribute name so we have to look up the id.
                All requests should now be cached via the requests_cache lib.
            """
            ...
            message = 'Invalid attribute identifier in check_attribute()'
    ...
    """
        Entities are any of the objects stored underneath a campaign,
        e.g. Characters, Locations, etc...
    """
    def get_entity(self, entity_type, entity):
        ...
        url = self.base_url + '/campaigns/' + str(self.campaign_id) + '/'
    ...
    def get_entity_id(self, entity_type, entity_name):
        """
           ...
           - Right now this is an exact case-sensitive match.
           This can be made more fuzzy
        """
    ...
    """
        ...
        or manual addition.  For my puproses these are game mechanics and
        things off of a character sheet.
        Notes:
        - I may just use the entity get call to retrieve attributes since they
        should be a subset of the entity.
    """
    def get_attribute(self, entity_type, entity, attribute):
        ...
        url = self.base_url + '/' + str(self.campaign_id) + '/entities/'
        + str(entity_id) + '/attributes/' + str(attribute_id)
    ...
    def get_attributes(self, entity_id):
        ...
        """
        url = self.base_url + '/campaign/' + str(self.campaign_id)
        + '/entities/' + str(entity_id) + '/attributes'

        return json.loads(requests.get(url, headers=self.headers))

    def get_attribute_id(self, entity_type, entity, attribute_name):
        """
            Like for entities, you need a numeric id to look up specific
            entries
        """
        entity_id = self.check_entity(entity_type, entity)
    ...
    def update_attribute(self, entity_type, entity, attribute_name, value):
        """
            Just going with bare minimum for post body.  There's important
            other possible fields that should be supported.
        """
        entity_id = self.check_entity(entity_type, entity)
        attribute_id = self.check_attribute(entity_type,
                                            entity_id,
                                            attribute_name)
        ...
        url = self.base_url + '/campaign/' + str(self.campaign_id)
        + '/entities/' + str(entity_id) + '/attributes/' + str(attribute_id)
    ...
    def update_attributes(self, entity_type, entity, attributes_and_values):
        """
            Plural attribute setter.  Takes a dictionary
            e.g. {'attribute name': 'attribute value'}
        """
    ...
    def update_entities(self, entity_type, entities):
        """
            Plural entity setter.  Takes a dictionary
            e.g. {'entity name': {<entity body here>}}
        """
    ...
    def delete_attribute(self, entity_type, entity, attribute):
        entity_id = self.check_entity(entity_type, entity)
        attribute_id = self.check_attribute(entity_type, entity_id, attribute)
        url = self.base_url + '/campaign/'
        + str(self.campaign_id) + '/entities/' + str(entity_id)
